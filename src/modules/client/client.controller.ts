...
export class ClientController {
  ...
  @Post('/quickCreate')
  @ApiOperation({
    summary: 'Criar cliente rapidamente ou retornar cliente existente'
  })
  ...
  async quickCreateClient(@Body() clientData: QuickCreateClientDto) {
    ...
    const existingClient = await this.clientService.findClientByName(
      clientData.name
    )
    ...
    const newClient = await this.clientService.quickCreateClient(
      clientData.name,
